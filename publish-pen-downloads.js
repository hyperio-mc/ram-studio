/**
 * publish-pen-downloads.js
 * Publishes .pen files as HTML pages with a working download button.
 * The .pen JSON is embedded in the page — clicking download saves it as a .pen file.
 */
...
function buildPage(config) {
  const { penFile, filename, title, subtitle, screens, palette, slug } = config;
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .btn-download{display:block;width:100%;padding:14px 24px;background:${palette.primary};border:none;border-radius:12px;color:#fff;font-size:15px;font-weight:700;cursor:pointer;transition:opacity .15s;margin-bottom:12px;text-decoration:none}
  ...
  .success-msg{display:none;color:#00cc66;font-size:12px;margin-top:4px}
</style>
</head>
<body>
<div class="card">
  ...
  <div class="meta">
    ...
    <div class="badge">size: <b>${sizeKB} KB</b></div>
  </div>
  ...
  <div class="instructions">
    ...
    <ol>
      <li>Download the <code>${filename}</code> file above</li>
      <li>In your IDE (Cursor / VS Code / Claude Code), open the Pencil panel</li>
      <li>Use <strong>File → Open</strong> and select the downloaded file</li>
    </ol>
  </div>
</div>
...
async function main() {
  ...
  console.log('Publishing .pen download pages...\n');
  for (const p of pages) {
    const html = buildPage(p);
    const r = await post(p.slug, p.title, html);
    const icon = r.status === 201 ? '✅' : r.status === 409 ? '⚠️  slug taken' : '❌';
    console.log(`${icon} ${p.slug} — HTTP ${r.status}  (${(html.length/1024).toFixed(0)} KB page)`);
