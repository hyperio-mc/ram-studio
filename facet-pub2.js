...
async function main() {
  const heroHtml = fs.readFileSync('/workspace/group/design-studio/facet-hero.html', 'utf8');

  console.log('1. Publishing hero...');
  const h = await publishToZenbin(SLUG, `${APP_NAME} — ${TAGLINE}`, heroHtml);
  console.log(`   ${h.status}:`, h.body.slice(0,120));
  ...
  console.log(`   ${v.status}:`, v.body.slice(0,120));
