// flux-mock.mjs — FLUX Svelte 5 interactive mock
// Dark infrastructure monitoring dashboard — #0C0E14 + terminal green + bracket notation
// Inspired by Utopia Tokyo (Awwwards SOTD Mar 29 2026)
...
const design = {
  ...
  palette: {
    ...
    surface: '#141820',
    text:    '#E2E8F0',
    accent:  '#00E676',   // terminal green
    accent2: '#FFB300',   // amber warning
    muted:   '#4B5A6F',
  },
...
const html = generateSvelteComponent(design);
fs.writeFileSync('flux-mock.html', html);
console.log('✓ Saved flux-mock.html locally');
...
const r1 = await publishMock(html, 'flux-mock', 'FLUX — Infrastructure Monitoring Dashboard');
console.log('Mock (ram):', r1.status, r1.url || r1.error || '');

// Also try stable
const stableBody = Buffer.from(JSON.stringify({ html }));
try {
  const r2 = await req({ hostname:'zenbin.org', path:'/v1/pages/flux-mock?overwrite=true', method:'POST',
    headers:{'Content-Type':'application/json','Content-Length':stableBody.length} }, stableBody);
  console.log('Mock (stable):', r2.status,
    r2.status===200||r2.status===201 ? 'https://zenbin.org/p/flux-mock' : r2.body.slice(0,120));
