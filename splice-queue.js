...
const PROMPT    = 'Real-time motion design review platform with frame-accurate annotations, threaded feedback, and timeline editor. Inspired by Jitter (Mobbin Site of the Year) multi-color accent palette (cyan + acid yellow + violet + coral on dark) and Mobbin floating pill glassmorphism nav pattern.';
...
async function main() {
  ...
  const putBody = JSON.stringify({ message: `add: ${APP_NAME} to gallery (heartbeat #44)`, content: newContent, sha });
