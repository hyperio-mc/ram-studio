/**
 * Build the CODA mock HTML and save locally, then publish
 */
...
function publishPage(slug, html, title) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        console.log(`  Status ${res.statusCode}: ${d.slice(0, 150)}`);
        resolve({ status: res.statusCode, body: d, url: `https://ram.zenbin.org/${slug}` });
      });
    });
    req.on('error', reject);
...
// Save locally first
writeFileSync('/workspace/group/design-studio/coda-mock.html', html);
console.log('Saved locally: coda-mock.html');

// Publish
const result = await publishPage('coda-mock', html, 'CODA — Financial Intelligence · Interactive Mock');
if (result.status === 200 || result.status === 201) {
  console.log('Mock live at:', result.url);
} else {
  console.log('Publish returned status:', result.status, '— mock saved locally at coda-mock.html');
  console.log('Note: Mock HTML is saved. ZenBin page limit reached.');
}
